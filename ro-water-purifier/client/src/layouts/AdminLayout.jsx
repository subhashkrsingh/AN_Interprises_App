...
import { useListResourceQuery, useLogoutMutation } from '../features/api/adminApi.js';
...
function SidebarContent({ onNavigate }) {
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Toolbar sx={{ px: 3 }}>
        <Stack>
          <Typography variant="h6">Commerce Admin</Typography>
...
export default function AdminLayout() {
  ...
  const { data: notifications } = useListResourceQuery(
    { resource: 'notifications', limit: 20 },
    { pollingInterval: 30000, refetchOnFocus: true, refetchOnReconnect: true }
  );
  const unreadNotifications = notifications?.data?.filter((item) => !item.isRead).length || 0;
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      >
        <Toolbar sx={{ gap: 1.5 }}>
          ...
          <Avatar sx={{ width: 36, height: 36 }}>{user?.name?.charAt(0) || 'A'}</Avatar>
